...
fn skew_constant<T: Float>() -> T { math::cast(0.36602540378) } // 0.5 * (sqrt(3.0) - 1.0)
fn unskew_constant<T: Float>() -> T { math::cast(0.2113248654) } // (3.0 - sqrt(3.0)) / 6.0

fn simplex_size2<T: Float>() -> T { math::cast(0.70710678118) } // sqrt(0.5)
fn simplex_size3<T: Float>() -> T { math::cast(0.61237243569) } // sqrt(simplex_size2() ^ 2 + (simplex_size2() / 2.0) ^ 2)
fn simplex_size4<T: Float>() -> T { math::cast(0.53033008588) } // sqrt(simplex_size3() ^ 2 + (simplex_size3() / 2.0) ^ 2)

fn simplex_size2_sqr<T: Float>() -> T { math::pow2(simplex_size2()) }
fn simplex_size3_sqr<T: Float>() -> T { math::pow2(simplex_size3()) }
fn simplex_size4_sqr<T: Float>() -> T { math::pow2(simplex_size4()) }

fn simplex_size3_inv<T: Float>() -> T { math::cast::<_,T>(1) / simplex_size3() } // 1 / simplex_size2()
fn simplex_size4_inv<T: Float>() -> T { math::cast::<_,T>(1) / simplex_size4() } // 1 / simplex_size2()

fn layer_offset_x<T: Float>() -> T { math::cast(0.45534180126) } // (2.0 - 3.0 * unskew_constant()) / 3.0
fn layer_offset_y<T: Float>() -> T { math::cast(0.12200846793) } // (1.0 - 3.0 * unskew_constant()) / 3.0
fn layer_offset_z<T: Float>() -> T { math::cast(0.35355339059) } // (1.0 - 3.0 * unskew_constant()) / 3.0

fn norm2_constant<T: Float>() -> T { math::cast(99.0) }
fn norm3_constant<T: Float>() -> T { math::cast(390.0) }
fn norm4_constant<T: Float>() -> T { math::cast(1400.0) }
...
fn simplectic3_points<T: Float>(point: &math::Point3<T>) -> [SimplecticPoint3<T>; 6] {
    let layer = (point[2] * simplex_size3_inv()).floor();
    ...
    let z_offset = point[2] - layer * simplex_size3();
    [
        ...
        p4.to_simplectic_point3(layer_int + 1, z_offset - simplex_size3()),
        p5.to_simplectic_point3(layer_int + 1, z_offset - simplex_size3()),
        p6.to_simplectic_point3(layer_int + 1, z_offset - simplex_size3()),
    ]
}

#[inline(always)]
fn simplectic4_points<T: Float>(point: &math::Point4<T>) -> [SimplecticPoint4<T>; 12] {
    let layer = (point[3] * simplex_size4_inv()).floor();
    let layer_int: i64 = math::cast(layer);
    ...
    let w_offset = point[3] - layer * simplex_size4();
    [
        ...
        p7.to_simplectic_point4(layer_int + 1, w_offset - simplex_size4()),
        p8.to_simplectic_point4(layer_int + 1, w_offset - simplex_size4()),
        p9.to_simplectic_point4(layer_int + 1, w_offset - simplex_size4()),
        p10.to_simplectic_point4(layer_int + 1, w_offset - simplex_size4()),
        p11.to_simplectic_point4(layer_int + 1, w_offset - simplex_size4()),
        p12.to_simplectic_point4(layer_int + 1, w_offset - simplex_size4()),
    ]
}
...
pub fn simplectic2<T: Float>(seed: &Seed, point: &math::Point2<T>) -> T {
    ...
    fn gradient<T: Float>(seed: &Seed, p: &SimplecticPoint2<T>) -> T {
        let attn = simplex_size2_sqr::<T>() - math::dot2(p.offset, p.offset);
        if attn > math::cast(0) {
...
pub fn simplectic3<T: Float>(seed: &Seed, point: &math::Point3<T>) -> T {
    ...
    fn gradient<T: Float>(seed: &Seed, p: &SimplecticPoint3<T>) -> T {
        let attn = simplex_size3_sqr::<T>() - math::dot3(p.offset, p.offset);
        if attn > math::cast(0) {
...
pub fn simplectic4<T: Float>(seed: &Seed, point: &math::Point4<T>) -> T {
    ...
    fn gradient<T: Float>(seed: &Seed, p: &SimplecticPoint4<T>) -> T {
        let attn = simplex_size4_sqr::<T>() - math::dot4(p.offset, p.offset);
        if attn > math::cast(0) {
